...
itgCalendar.prototype = {
	...
	_init: function(params) {
		params = Params.parse(params, { showEvents: true,showWorldTimes:true });
		...
		if (params.showWorldTimes) {
			let separator = new PopupMenu.PopupSeparatorMenuItem();
			separator.setColumnWidths(1);
			vbox.add(separator.actor, {y_align: St.Align.END, expand: true, y_fill: false});

			displayDate = new Date();
			vbox.add(new St.Label({ style_class: 'events-day-header', text: displayDate.toLocaleFormat(DATE_FORMAT) }));
	...
	enable: function() {
		Main.panel._centerBox.insert_actor(this._upClient, 0);
	},

	disable: function() {
		Main.panel._centerBox.remove_actor(this._upClien);
	}
}
